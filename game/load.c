...
/*   Updated: 2025/04/07 20:08:08 by ymouchta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	var->imgs.load_background = mlx_texture_to_image(var->mlx_init,
			var->load.load_background);
	var->imgs.load_player = mlx_texture_to_image(var->mlx_init,
			var->load.load_player);
	var->imgs.load_wall = mlx_texture_to_image(var->mlx_init,
			var->load.load_wall);
	var->imgs.load_coin = mlx_texture_to_image(var->mlx_init,
			var->load.load_coin);
	var->imgs.load_exit_0 = mlx_texture_to_image(var->mlx_init,
			var->load.load_exit_0);
...
bool	check_texture_loading(t_mlx_game *var)
{
	set_texture(var);
	png_loading(var);
	if (!(var->load.load_background) || !(var->load.load_coin)
		|| !(var->load.load_exit_0) || !(var->load.load_exit_1)
		|| !(var->load.load_player) || !(var->load.load_wall))
	{
		clear_texture(var);
		return (false);
	}
